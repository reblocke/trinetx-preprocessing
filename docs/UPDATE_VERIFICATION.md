...
row order does not matter; multiplicity and missing values do. Only `run_id`
is excluded from table values; deterministic index-event IDs remain exact.
...
DuckDB spill. Treat these as private aggregate evidence, not publishable data.
...
automatic. Preserve an accepted baseline receipt to avoid rebuilding historical

...
def split_rows_by_code_groups(
    ...
    *,
    ...
    Regexes are evaluated once per unique code in the chunk, then rows are
    fanned out to every matching group. This preserves legacy overlapping
    outputs while avoiding repeated regex scans over every row.
    """
    ...
    compiled_groups = [(group, re.compile(group.regex)) for group in code_groups]
    matched_codes_by_group = {group.name: set[str]() for group in code_groups}

    for raw_code in codes.dropna().unique():
        code = str(raw_code)
        for group, pattern in compiled_groups:

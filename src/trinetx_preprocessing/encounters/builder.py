...
def build_encounters(
    *,
    ...
    from .coverage import AUDIT_DOMAINS, COVERAGE_POLICIES, COVERAGE_POLICY_VERSION

    if (
        not coverage.get("pass")
        or coverage.get("contract_version") != "1.0"
        or coverage.get("coverage_policy_version") != COVERAGE_POLICY_VERSION
        or coverage.get("coverage_policy") not in COVERAGE_POLICIES
        or (
            coverage.get("coverage_policy") == "approved_incomplete_linkage"
            and not coverage.get("approved_exception")
        )
        or set(coverage.get("variants", {})) != set(VARIANTS)
        or any(
            not item.get("pass")
            or item.get("coverage_policy") != coverage["coverage_policy"]
            or item.get("contradictions_pass") is not True
            or (
                coverage["coverage_policy"] == "complete_linkage"
                and (
                    item.get("complete_patient_linkage") is not True
                    or item.get("complete_encounter_linkage") is not True
                )
            )
            for item in coverage.get("variants", {}).values()
        )

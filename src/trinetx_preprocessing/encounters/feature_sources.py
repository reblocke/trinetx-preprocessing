"""Encounter-anchored measurements and evidence; no study eligibility decisions."""
...
def _build_diagnosis_evidence(
    ...
    connection.execute(
        ...
        JOIN source_diagnosis AS diagnosis
         ...
         AND diagnosis.event_datetime::DATE <= analysis.index_date
...
def _build_procedure_evidence(
    ...
    connection.execute(
        ...
        JOIN source_procedure AS procedure
         ...
         AND procedure.event_datetime::DATE <= analysis.index_date
        JOIN concept_set AS concept
...
def _build_lab_summary(
    ...
    measurement_lookback = config.measurement_lookback_days
    history_lookback = config.lookback_days
    ...
    connection.execute(
        ...
        JOIN normalized_component_lab AS lab
         ...
         AND lab.event_datetime::DATE <= analysis.index_date
...
def _build_blood_pressure_summary(
    ...
    connection.execute(
        ...
        JOIN source_vital_measurement AS vital
         ...
         AND vital.event_datetime::DATE <= analysis.index_date
         AND {in_lookback}
        LEFT JOIN encounter_context_source AS encounter
...
def _build_medication_evidence(
    ...
    lookback = config.medication_lookback_days
    followup = config.followup_days
    in_lookback = inclusive_lookback_start_sql(
    ...
    connection.execute(
        ...
        SELECT
            ...
            datediff('day', medication.event_datetime, analysis.index_date)
                AS days_before_index,
            medication.event_datetime::DATE <= analysis.index_date
                AND (
        ...
        WITH encounter AS (
            SELECT
                ...
                min(encounter.encounter_start) FILTER (
                    WHERE encounter.encounter_start::DATE <= analysis.index_date

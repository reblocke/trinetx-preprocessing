...
def default_catalog_directory() -> Path:
    """Locate the versioned source catalog in an installed or editable package."""
    installed = Path(__file__).resolve().parents[1] / "catalog"
    if installed.is_dir():
        return installed
    return Path(__file__).resolve().parents[3] / "config" / "concept_sets"
